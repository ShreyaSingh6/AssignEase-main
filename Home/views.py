...
from .utils.openai_helper import get_assignment_help
...
def Save_signup(request):
    ...
    signup_obj = Student(Roll=Roll, Name=Name, Email=Email, Password=Password, Gender=Gender, Mobile=Mobile, DOB=DOB,
                         Course=Course)
    signup_obj.save()
...
def Teacher_signup(request):
    ...
    signup_obj = Teacher(Roll=Roll, Name=Name, Email=Email, Password=Password, Gender=Gender, Mobile=Mobile, DOB=DOB,
                         Subject=Subject)
    signup_obj.save()
...
@csrf_exempt  # optional if CSRF token is passed correctly from frontend
def assignment_help_view(request):
    if request.method == 'POST':
        query = request.POST.get('query', '')
        if not query.strip():
            return JsonResponse({'response': 'Please enter a question.'})

        # Modify the query to instruct your AI to generate only a hint
        hint_prompt = f"Give only a  hint (not the full answer) for the following question:\n{query}"
        response = get_assignment_help(hint_prompt)

        return JsonResponse({'response': response})

    return render(request, 'assignment_help.html')
